module.exports = async (Sentry) => {
  ...
  const patreonAuth = (req, res, next) => {
    passport.authenticate("bearer", (err, user, info) => {
      if (
        (user && user.patronLevel && user.patronLevel > 0) ||
        env == "development"
      ) {
        req.user = user;
        next();
        ...
        res
          .status(401)
          .send(
            "This endpoint is in limited beta and requires a Patreon subscription."
          );
      }
    })(req, res, next);
  };
  ...
  const superPatreonAuth = (req, res, next) => {
    passport.authenticate("bearer", (err, user, info) => {
      if (
        (user && user.patronLevel && user.patronLevel > 1) ||
        env == "development"
      ) {
        req.user = user;
        next();
        ...
        res
          .status(401)
          .send(
            "This endpoint is in limited beta and requires a Patreon Tier 2 subscription."
          );
      }
    })(req, res, next);
  };
  ...
  app.use(
    swStats.getMiddleware({
      ...
      onAuthenticate: (req, username, password) => {
        return (
          username.toLowerCase() == process.env.USERNAME.toLowerCase() &&
  ...
  require("../app/rankings/rankings.route")(
    app,
    dbInfo.db,
    middlewares,
    Sentry
  ...
  require("../app/recruiting/recruiting.route")(
    app,
    dbInfo.db,
    middlewares,
    Sentry
  ...
  await require("../app/live/live.route")(
    app,
    dbInfo.db,
    patreonMiddlewares,
    Sentry
